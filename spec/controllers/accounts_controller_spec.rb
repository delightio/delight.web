  ...
  describe "POST 'create'" do
    ...
    describe "user already have an account" do 
      before(:each) do
        ...
        @admin.create_account(:name => 'account name')
        @admin.save!
        sign_in(@admin)
      end 

      it "should redirect user to account page" do 
        post 'create', { :account => { :name => 'account name' } }
        #response.should redirect_to(account_path(@admin.account))
        response.should redirect_to(:controller => 'accounts', :action => :show, :id => @admin.account.id)
      end 
    end 
  end
